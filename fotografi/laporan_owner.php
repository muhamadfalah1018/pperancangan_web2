<?php
// laporan_owner.php - Laporan Lengkap Versi Owner (Tabel & Rincian)
session_start();
include('includes/db_koneksi.php');

// 1. CEK AKSES KHUSUS OWNER
...
$user_fullname = $_SESSION['name'] ?? 'Owner';
...
// Checkbox Link (Khusus Hasil Foto)
...
// 3. LOGIKA QUERY (Mengambil data persis seperti Admin)
...
if ($is_excel) {
    ...
    header("Content-Disposition: attachment; filename=Laporan_Owner_{$jenis}_{$satuan}_" . date('Ymd') . ".xls");
...
<head>
    <meta charset="UTF-8">
    <title>Laporan Owner - <?php echo $jenis; ?></title>
    ...
    <style>
        /* --- STYLE GLOBAL (SERAGAM DENGAN DASHBOARD OWNER) --- */
        :root {
            --primary: #6A5ACD;
            --text-dark: #333;
            --text-gray: #666;
        ...
        /* Menu Owner: Hanya Dashboard & Laporan */
        ...
        .user-menu { margin-left: auto; font-weight: 600; font-size: 14px; color: #333; display: flex; align-items: center; gap: 15px; }
        .logout-btn { color: #dc3545; border: 1px solid #dc3545; padding: 5px 12px; border-radius: 20px; text-decoration: none; font-size: 12px; transition: 0.3s; }
...
<body>
    ...
    <div class="top-nav">
        ...
        <div class="nav-links">
            <a href="dashboard_owner.php">Dashboard Rincian</a>
            <a href="laporan_owner.php" class="active-link">Laporan Lengkap</a>
        </div>
        <div class="user-menu">
            <i class="fas fa-user-tie"></i> <?php echo htmlspecialchars($user_fullname); ?>
